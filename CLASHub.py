import csv, getopt, collections, os, re, sys, glob, subprocess, logging
import traceback
...
class Viennad_to_table():
    ...
    def file_Basepattern_VerticalBar(self, inputFile):
        ...
        outputFile = inputFile.replace("_region.txt","_VerticalBar.csv")
...
class Combined_table():
    ...
    def rnaseqTPM_merge(self, JobID1, species1):
        try:
            ...
            f_all = pd.DataFrame()
            for each_file in all_tpm_tables:
...
class Statistic(): ## CLA, CLASH fasta; CLQ, CLASH fastq; CUR, Cumulative fraction Curve; msq: miRNA-seq
    ...
    def miSeq_fasta_log(self, JobID1, Output_filename1):
        ...
        with open(f"/pubapps/mingyi.xie/clashhub/prod/app/Sbatch_documents/{JobID1}.log") as f1:
            allines = f1.read()
            log_data = {}
    ...
    def aqPE_data_report(self, JobID1):
        try:
            ...
            with open(f"/pubapps/mingyi.xie/clashhub/prod/app/Sbatch_documents/{JobID1}.log") as log_file:
    ...
    def CLASH_fastq_log(self,JobID1, Output_filename1):
        ...
        # 生成rna type HTML表格
        f1 = pd.read_csv(f"{Output_filename1}_VerticalBar.csv")
    ...
    def Deseq2_data_report(self, JobIDinput):
        ...
        with open(f"/pubapps/mingyi.xie/clashhub/prod/app/Sbatch_documents/{jobID}.log", 'r') as log_file:# 读取日志文件内容
            log_content = log_file.read()
        ...
        adapter_3_prime = re.findall(r'RNAseq_(?:control|treatment)_THREE_PRIME_ADAPTER_\d+=(\w+)', log_content,
                                     re.MULTILINE)
        input1_files = re.findall(r'RNAseq_(?:control|treatment)_INPUT_FILE1_\d+=.*/(\w+\.fastq\.gz)', log_content,
                                  re.MULTILINE)
        input2_files = re.findall(r'RNAseq_(?:control|treatment)_INPUT_FILE2_\d+=.*/(\w+\.fastq\.gz)', log_content,
                                  re.MULTILINE)
    ...
    def GeneTPM_data_report(self, JobIDinput):
        try:
            ...
            adapter_3_prime = re.findall(r'RNAseq_THREE_PRIME_ADAPTER_\d+=(\w+)', log_content,
                                         re.MULTILINE)
            input1_files = re.findall(r'RNAseq_INPUT_FILE1_\d+=.*/(\w+\.fastq\.gz)', log_content,
                                      re.MULTILINE)
            input2_files = re.findall(r'RNAseq_INPUT_FILE2_\d+=.*/(\w+\.fastq\.gz)', log_content,
                                      re.MULTILINE)
    ...
    def Cumulative_fraction_curve(self,DeseqInput="", BaseMeanFilterNumber="100",
        ...
        try:
            ...
            if species == "Human":
                ...
                f_CLASH = pd.read_csv("/pubapps/mingyi.xie/clashhub/prod/app/CummulativeCruve_TargetInfo/Celegans_CLASH_each_miRNA_targets_20241006.csv",index_col=0)
        ...
        try:
            if argv_step_command == 'making_unique_redundant_database_hg38':
                ...
                try:
                    ...
                    print("\nStep V: Change BasePattern to VerticalBar, 延长genome position，与target element 位置匹配，不是仅仅包含pairing的position, ~2 minutes")
                    Viennad_to_table().file_Basepattern_VerticalBar(inputFile=f"{input1}_region.txt")
                except Exception as e:
                    traceback.print_exc()
                    sys.exit()
